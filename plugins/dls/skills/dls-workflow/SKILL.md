...
At the start of a standard or critical implementation request, including Plan Mode, perform a bounded readiness preflight before reading implementation source, specifications beyond their identifiers, or tests: verify plugin-local provenance, call `delivery-status`, and only when its single typed action permits implementation call `context --phase implementation --dry-run`. If the action is `continue-definition`, `wait-dependency`, `rebase-after-dependency`, `approve-definition`, or another non-implementation boundary, stop immediately and return that one action. Do not inspect product source, run baseline tests, ask helpers, loop over CLI help, or manually reconstruct state while preflight is blocked. Plan Mode does not authorize speculative implementation discovery across a failed DLS boundary.
