...
- The M1 exit record is appended only after EF-01 is accepted and its receipt
  provides the exact evidence digests.
...
- `REQ-002`: The fast public check rejects a missing required claim or missing
  mapped test symbol; behavior remains proven by the existing full suite.
